...
instance PgrstError ApiRequestError where
  ...
  status _ SpreadNotToOne{}            = HTTP.status400
  ...
  code QueryParamError{}           = ApiRequestErrorCode00
  code InvalidRpcMethod{}          = ApiRequestErrorCode01
  code InvalidBody{}               = ApiRequestErrorCode02
  code InvalidRange{}              = ApiRequestErrorCode03
  code InvalidFilters              = ApiRequestErrorCode05
  code UnacceptableSchema{}        = ApiRequestErrorCode06
  code MediaTypeError{}            = ApiRequestErrorCode07
  code NotEmbedded{}               = ApiRequestErrorCode08
  code GucHeadersError             = ApiRequestErrorCode11
  code GucStatusError              = ApiRequestErrorCode12
  code PutLimitNotAllowedError     = ApiRequestErrorCode14
  code PutMatchingPkError          = ApiRequestErrorCode15
  code SingularityError{}          = ApiRequestErrorCode16
  code UnsupportedMethod{}         = ApiRequestErrorCode17
  code RelatedOrderNotToOne{}      = ApiRequestErrorCode18
  code SpreadNotToOne{}            = ApiRequestErrorCode19
  code UnacceptableFilter{}        = ApiRequestErrorCode20
  code PGRSTParseError{}           = ApiRequestErrorCode21
  code InvalidPreferences{}        = ApiRequestErrorCode22
  code AggregatesNotAllowed{}      = ApiRequestErrorCode23
  code MaxAffectedViolationError{} = ApiRequestErrorCode24
  code InvalidResourcePath         = ApiRequestErrorCode25
  code OpenAPIDisabled             = ApiRequestErrorCode26

  code NoRelBetween{}              = SchemaCacheErrorCode00
  code AmbiguousRelBetween{}       = SchemaCacheErrorCode01
  code NoRpc{}                     = SchemaCacheErrorCode02
  code AmbiguousRpc{}              = SchemaCacheErrorCode03
  code ColumnNotFound{}            = SchemaCacheErrorCode04
  code TableNotFound{}             = SchemaCacheErrorCode05
...
instance JSON.ToJSON ApiRequestError where
  toJSON err@(QueryParamError (QPError message details)) = toJsonPgrstError
    (code err) message (Just (JSON.String details)) Nothing

  toJSON err@(InvalidRpcMethod method) = toJsonPgrstError
    (code err) ("Cannot use the " <> T.decodeUtf8 method <> " method on RPC") Nothing Nothing

  toJSON err@(InvalidBody errorMessage) = toJsonPgrstError
    (code err) (T.decodeUtf8 errorMessage) Nothing Nothing

  toJSON err@(InvalidRange rangeError) = toJsonPgrstError
    (code err)
  ...
  toJSON err@InvalidFilters = toJsonPgrstError
    (code err) "Filters must include all and only primary key columns with 'eq' operators" Nothing Nothing

  toJSON err@(UnacceptableSchema schemas) = toJsonPgrstError
    (code err) ("The schema must be one of the following: " <> T.intercalate ", " schemas) Nothing Nothing

  toJSON err@(MediaTypeError cts) = toJsonPgrstError
    (code err) ("None of these media types are available: " <> T.intercalate ", " (map T.decodeUtf8 cts)) Nothing Nothing

  toJSON err@(NotEmbedded resource) = toJsonPgrstError
    (code err)
  ...
  toJSON err@GucHeadersError = toJsonPgrstError
    (code err) "response.headers guc must be a JSON array composed of objects with a single key and a string value" Nothing Nothing

  toJSON err@GucStatusError = toJsonPgrstError
    (code err) "response.status guc must be a valid status code" Nothing Nothing

  toJSON err@PutLimitNotAllowedError = toJsonPgrstError
    (code err) "limit/offset querystring parameters are not allowed for PUT" Nothing Nothing

  toJSON err@PutMatchingPkError = toJsonPgrstError
    (code err) "Payload values do not match URL in primary key column(s)" Nothing Nothing

  toJSON err@(SingularityError n) = toJsonPgrstError
    (code err)
    "Cannot coerce the result to a single JSON object"
    (Just $ JSON.String $ T.unwords ["The result contains", show n, "rows"])
    Nothing

  toJSON err@(UnsupportedMethod method) = toJsonPgrstError
    (code err) ("Unsupported HTTP method: " <> T.decodeUtf8 method) Nothing Nothing

  toJSON err@(RelatedOrderNotToOne origin target) = toJsonPgrstError
    (code err)
  ...
  toJSON err@(InvalidPreferences prefs) = toJsonPgrstError
    (code err)
    "Invalid preferences given with handling=strict"
    (Just $ JSON.String $ T.decodeUtf8 ("Invalid preferences: " <> BS.intercalate ", " prefs))
    Nothing

  toJSON err@AggregatesNotAllowed = toJsonPgrstError
    (code err) "Use of aggregate functions is not allowed" Nothing Nothing

  toJSON err@(MaxAffectedViolationError n) = toJsonPgrstError
    (code err)
    "Query result exceeds max-affected preference constraint"
    (Just $ JSON.String $ T.unwords ["The query affects", show n, "rows"])
    Nothing

  toJSON err@InvalidResourcePath = toJsonPgrstError
    (code err)
    "Invalid path specified in request URL"
    Nothing
    Nothing

  toJSON err@OpenAPIDisabled = toJsonPgrstError
    (code err)
    "Root endpoint metadata is disabled"
    Nothing
    Nothing

  toJSON err@(NoRelBetween parent child embedHint schema allRels) = toJsonPgrstError
    (code err)
  ...
  toJSON err@(AmbiguousRelBetween parent child rels) = toJsonPgrstError
    (code err)
  ...
  toJSON err@(NoRpc schema procName argumentKeys contentType isInvPost allProcs overloadedProcs)  =
  ...
  toJSON err@(AmbiguousRpc procs)  = toJsonPgrstError
    (code err)
  ...
  toJSON err@(ColumnNotFound relName colName) = toJsonPgrstError
    (code err) ("Could not find the '" <> colName <> "' column of '" <> relName <> "' in the schema cache") Nothing Nothing

  toJSON err@(TableNotFound schemaName relName tbls) = toJsonPgrstError
    (code err)
...
data PgError = PgError SQL.UsageError
...
instance PgrstError PgError where
  ...
  code (PgError usageError) = code usageError
...
instance PgrstError SQL.UsageError where
  ...
  code (SQL.ConnectionUsageError _)      = ConnectionErrorCode00
  code SQL.AcquisitionTimeoutUsageError  = ConnectionErrorCode03
  code (SQL.SessionUsageError (SQL.QueryError _ _ commandError)) = code commandError
...
instance PgrstError SQL.CommandError where
  ...
  code (SQL.ClientError _)      = ConnectionErrorCode01
  code (SQL.ResultError rError) = code rError

instance PgrstError SQL.ResultError where
  status authed rError = pgResultErrorStatus authed rError
  ...
  code (SQL.ServerError "PGRST" m d _ _) =
    case parseRaisePGRST m d of
      Right (r, _) -> PgErrorCode (getCode r)
      Left e       -> code e

  code (SQL.ServerError c _ _ _ _) = PgErrorCode (T.decodeUtf8 c)

  -- This is a fatal error, we kill the main thread on this error
  code _ = InternalErrorCode00
...
instance JSON.ToJSON SQL.UsageError where
  toJSON err@(SQL.ConnectionUsageError e) = toJsonPgrstError
    (code err)
  ...
  toJSON err@SQL.AcquisitionTimeoutUsageError = toJsonPgrstError
    (code err) "Timed out acquiring connection from connection pool." Nothing Nothing
...
instance JSON.ToJSON SQL.CommandError where
  toJSON err@(SQL.ClientError d) = toJsonPgrstError
    (code err) "Database client error. Retrying the connection." (JSON.String <$> fmap T.decodeUtf8 d) Nothing
...
instance JSON.ToJSON SQL.ResultError where
  ...
  toJSON err = toJsonPgrstError
    (code err) (show err) Nothing Nothing
...
instance PgrstError Error where
  ...
  code NoSchemaCacheError    = ConnectionErrorCode02
...
instance PgrstError JwtError where
  ...
  code JwtSecretMissing   = JWTErrorCode00
  code (JwtDecodeError _) = JWTErrorCode01
  code JwtTokenRequired   = JWTErrorCode02
  code (JwtClaimsError _) = JWTErrorCode03

instance JSON.ToJSON Error where
  toJSON (ApiRequestError err)  = JSON.toJSON err
  toJSON (JwtErr err)           = JSON.toJSON err
  toJSON (PgErr err)            = JSON.toJSON err
  toJSON err@NoSchemaCacheError = toJsonPgrstError
      (code err) "Could not query the database for the schema cache. Retrying." Nothing Nothing

-- Should we provide hints and description or explain the error in docs or both?
instance JSON.ToJSON JwtError where
  toJSON err@JwtSecretMissing = toJsonPgrstError
      (code err) "Server lacks JWT secret" Nothing Nothing

  toJSON err@(JwtDecodeError message) = toJsonPgrstError
      (code err) message Nothing Nothing

  toJSON err@JwtTokenRequired = toJsonPgrstError
      (code err) "Anonymous access is disabled" Nothing Nothing

  toJSON err@(JwtClaimsError message) = toJsonPgrstError
      (code err) message Nothing Nothing
