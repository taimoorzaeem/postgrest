...
lookupJwtCache jwtCacheState token maxLifetime parseJwt utc observer = do
  ...
  case (authResult,checkCache) of
    ...
    (Right res, Nothing) -> do -- cache miss
      ...
      C.insert' (jwtCache jwtCacheState) (Just timeSpec) token res

      -- calculate cache size with debounce and log it
  ...
  case cSizeDebouncer of
    ...
    where
      ...
      calculateSizeThenLog = do
        size <- calcCacheSizeInBytes (jwtCache jwtCacheState)
...
calcCacheSizeInBytes cache = do
  cacheList <- C.toList cache
