...
getAction resource schema method mediaType =
  case (resource, method) of
    ...
    (ResourceRelation rel, "PATCH")   -> case mediaType of -- we support JSON Patch on json-patch content-type
        MTVndJSONPatch -> Right . ActDb $ ActRelationMut  (qi rel) MutationJsonPatch
